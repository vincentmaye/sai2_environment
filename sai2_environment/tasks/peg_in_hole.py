...
class PegInHole(Task):
    ...
    def compute_reward(self):
        if self._simulation:
            ...
            elif (self.hd + diff_ee_hole[2] <= self.epsilon2) and (np.linalg.norm(diff_ee_hole[:2]) <= self.epsilon2): # Success if the distance between hole entry and peg bottom equals the hole depth minus some threshhold (epsilon2) and the difference in x-y plane is smaller than some threshold (not next to the hole)
                reward = 5
                done = True
            else:
