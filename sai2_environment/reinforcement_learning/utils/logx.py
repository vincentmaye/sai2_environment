...
class Logger:
    ...
    def __init__(self, output_dir=None, output_fname='progress.txt', exp_name=None):
        ...
        #self.video_dir = utils.make_dir(os.path.join(log_dir, 'video'))
        #self.fig_dir = utils.make_dir(os.path.join(log_dir, 'fig'))
        #self.model_dir = utils.make_dir(os.path.join(log_dir, 'model'))
        #self.buffer_dir = utils.make_dir(os.path.join(log_dir, 'buffer'))
...
class EpochLogger(Logger):
    ...
    def log_tabular(self, key, val=None, with_min_and_max=False, average_only=False, tensorboard=True, epoch=0):
        """
        ...
        if val is not None:
            ...
            super().log_tabular(key if average_only else 'Average' + key, stats[0], tensorboard = tensorboard, epoch = epoch)
            if not(average_only):
                super().log_tabular('Std'+key, stats[1], tensorboard =tensorboard, epoch = epoch)
            if with_min_and_max:
                super().log_tabular('Max'+key, stats[3], tensorboard =tensorboard, epoch = epoch)
                super().log_tabular('Min'+key, stats[2], tensorboard =tensorboard, epoch = epoch)
        self.epoch_dict[key] = []

    def log_tensorboard(self, key, val = None, with_min_and_max=False, average_only=False):
            """
